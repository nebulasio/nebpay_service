package io.nebulas.dapp.pay.common.web;
...
public class JsonpFastJsonHttpMessageConverter extends FastJsonHttpMessageConverter {
  ...
  protected void writePrefix(HttpOutputMessage outputMessage, Object object) throws IOException {
    ...
    byte[] bytes = sb.toString().getBytes(getFastJsonConfig().getCharset());
  ...
  protected void writeSuffix(HttpOutputMessage outputMessage, Object object) throws IOException {
    String jsonpFunction = object instanceof MappingJacksonValue ? ((MappingJacksonValue) object).getJsonpFunction() : null;
    if (jsonpFunction != null) {
      outputMessage.getBody().write(");".getBytes(getFastJsonConfig().getCharset()));
    }
  }
