...
import io.nebulas.dapp.pay.service.PayRecordService;
import io.nebulas.dapp.pay.service.thirdparty.neb.NebApiServiceWrapper;
 ...
 */
@Slf4j
@RequestMapping("/api/mainnet/pay")
@RestController
...
//    private RedisService redisService;
    ...
    public JsonResult save(@RequestParam("payId") String payId, @RequestParam("txHash") String txHash) {
        ...
        PayRecord record = payRecordService.getRecordByPayId(payId);
        if (null != record) {
...
//        String key = String.format("mainnet:payId:%s:%s", payId, LocalDateTime.now().toString("yyyyMMddHHmm"));
